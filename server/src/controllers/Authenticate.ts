import { IAdminModel, ILocationModel, IUserModel, TypedRequest } from "../types";
...
async function getUserInfo(req: TypedRequest<boolean>, res: Response) {
  if (req.body === false) {
    try {
      const { username, age, experience, bio, email } = req.user as InstanceType<IUserModel>;
      const locations: InstanceType<ILocationModel>[] = await Location.find();
      console.log({ userInfo: { username, email, age, experience, bio }, locations: locations.map(location => location.name) })
      res.status(200).send({ userInfo: { username, email, age, experience, bio }, locations: locations.map(location => location.name) });
    } catch (error) {
      res
        .status(500)
    ...
    try {
      const { username, location, email } = req.user as InstanceType<IAdminModel>;
